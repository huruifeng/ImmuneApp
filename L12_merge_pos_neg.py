...
ligand_pos_df = pd.read_csv("data/mhc_ligand_pos.csv",index_col=None,header=0)
...
neg_df.to_csv("data/mhc_ligand_neg.csv",index=False)

neg_df.loc[:,"label"] = 0

all_df = pd.concat([ligand_pos_df,neg_df])
all_df.to_csv("data/mhc_ligand_pos_neg.csv",index=False)
